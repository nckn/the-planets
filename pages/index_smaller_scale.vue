...
export default {
  ...
  methods: {
    ...
    addSunLight() {
      ...
      var sunOffsetY = 6
      var light = new THREE.PointLight( 0xffd88c, 0, 10 ); // orange: ffa800
      ...
      var divi = 0.5;
      light.shadow.mapSize.width = 1024 * divi;
      light.shadow.mapSize.height = 1024 * divi;
    ...
    actualScaleOfObjects() {
      ...
      self.planetList.forEach((snd, index) => {
        ...
        if (index === 0) {
          ...
          pre = self.planetList[index - 1].r
        }

        sum += pre

        // var s = snd.r[self.actualSize === true ? 0 : 1]
        var s = self.actualSize === true ? snd.s[0] : 1
        var nX = ((sum + index) * 1) + (s * 2) 
        ...
        // var nY = s
        var nY = self.planetList[index].r
    ...
    createPlanets () {
      var self = this 
      var mass = 5
      console.log('inside cratePlanets', self.planetList)
      // var shape = new CANNON.Box(new CANNON.Vec3(bS, bS, bS))
      self.noOFCubes = self.planetList.length
      self.planetList.forEach((planet, i) => {
        var shape;
        if (planet.type === 'sphere') {
          shape = new CANNON.Sphere(bS)
          // alert('is checking this')
        ...
        if (planet.type === 'sun') {
          ...
          rY = planet.r[0]; // 0 if on same z height w/ sun
          rZ = Math.cos( i / (self.noOFCubes - 1) * Math.PI * 2 ) * ringSize;
        }

        console.log('number of times running this')
        ...
        // return
        planetGeom = new THREE.SphereBufferGeometry( planet.r, reso, reso )
        ...
        planet.audio = sound
        var sndPath = path + planet.filename
        ...
        // Load text
        if (shouldShowText) {
          self.loadText(i, planet.name, {x: rX, y: rY, z: rZ})
        }
        
        // console.log('it is 2, ', sndPath)

        // if (i === 0) {
        //   console.log('it is 2, ', path + planet)
